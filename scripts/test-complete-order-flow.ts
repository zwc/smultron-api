...
async function getProduct(token: string, productId: string): Promise<Product | null> {
  ...
  if (!response.ok) {
    throw new Error(`Failed to get product: ${response.status} ${await response.text()}`);
  }
...
async function createProduct(token: string, productData: any): Promise<Product> {
...
async function updateProduct(token: string, productId: string, updates: Partial<Product>): Promise<Product> {
  ...
  if (!response.ok) {
    throw new Error(`Failed to update product: ${response.status} ${await response.text()}`);
  }

  const result = await response.json() as { data: Product };
  logSuccess(`Product updated: ${result.data.title} (Stock: ${result.data.stock})`);
  return result.data;
}

async function ensureProductsExist(token: string): Promise<Map<string, Product>> {
  ...
  for (const cartItem of TEST_ORDER.cart) {
    ...
    if (existingProduct) {
      ...
      if (existingProduct.stock < cartItem.number + 5) {
        logWarning(`Product ${existingProduct.title} has low stock (${existingProduct.stock}), updating...`);
        ...
        logSuccess(`Product ${existingProduct.title} has sufficient stock (${existingProduct.stock})`);
      ...
      const created = await createProduct(token, {
        ...
        categorySlug: cartItem.category,
...
async function placeOrder(token: string, cart?: any[]): Promise<CheckoutResponse> {
...
async function verifyStockReduction(token: string, expectedReductions: Map<string, number>): Promise<void> {
  ...
  for (const [productId, expectedReduction] of expectedReductions) {
    ...
    logInfo(`Product ${product.title}: Current stock = ${product.stock} (reservations active, not yet reduced)`);
  }
}

async function simulateSwishCallback(orderNumber: string, status: 'PAID' | 'DECLINED'): Promise<void> {
...
async function getOrder(token: string, orderId: string): Promise<any> {
  ...
  if (!response.ok) {
    throw new Error(`Failed to get order: ${response.status} ${await response.text()}`);
  }
...
async function main() {
  ...
  try {
    ...
    log(`  Total Amount: ${(TEST_ORDER.cart.reduce((sum, item) => sum + (item.price * item.number), 0) + TEST_ORDER.order.delivery_cost)} SEK`);
...
async function testDeclinedPayment() {
  ...
  try {
    ...
    if (updatedOrder.status === 'invalid') {
      ...
      logWarning(`Expected order status 'invalid', got '${updatedOrder.status}'`);
    }
