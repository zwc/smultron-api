...
export class SmultronStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: SmultronStackProps) {
    ...
    const adminUsername =
      this.node.tryGetContext('adminUsername') ||
      process.env.ADMIN_USERNAME ||
      'admin'
    const adminPassword =
      this.node.tryGetContext('adminPassword') || process.env.ADMIN_PASSWORD
    const jwtSecret =
      this.node.tryGetContext('jwtSecret') || process.env.JWT_SECRET
    const hostedZoneId =
      this.node.tryGetContext('hostedZoneId') || process.env.HOSTED_ZONE_ID

    if (!adminPassword || !jwtSecret) {
      throw new Error(
        'ADMIN_PASSWORD and JWT_SECRET must be provided via context or environment variables',
      )
    }
    ...
    if (environment === 'dev' && disableAuth === 'true') {
      ...
      console.warn(
        '⚠️  DISABLE_AUTH is enabled for dev environment - Authentication will be bypassed!',
      )
    }
    ...
    const listCatalogFunction = new lambda.Function(
      this,
      'ListCatalogFunction',
      {
        ...commonLambdaProps,
    ...
    const listProductsFunction = new lambda.Function(
      this,
    ...
    const adminListProductsFunction = new lambda.Function(
      this,
      'AdminListProductsFunction',
      {
        ...commonLambdaProps,
    ...
    const adminUpdateProductIndexesFunction = new lambda.Function(
      this,
      'AdminUpdateProductIndexesFunction',
      {
        ...commonLambdaProps,
    ...
    const listCategoriesFunction = new lambda.Function(
      this,
    ...
    const getCategoryFunction = new lambda.Function(
      this,
      'GetCategoryFunction',
      {
        ...commonLambdaProps,
    ...
    const createCategoryFunction = new lambda.Function(
      this,
    ...
    const updateCategoryFunction = new lambda.Function(
      this,
    ...
    const adminUpdateCategoryIndexesFunction = new lambda.Function(
      this,
      'AdminUpdateCategoryIndexesFunction',
      {
        ...commonLambdaProps,
    ...
    const createOrderFunction = new lambda.Function(
      this,
      'CreateOrderFunction',
      {
        ...commonLambdaProps,
    ...
    const updateOrderStatusFunction = new lambda.Function(
      this,
    ...
    swishCallbackFunction.addToRolePolicy(
      new iam.PolicyStatement({
        actions: ['ses:SendEmail', 'ses:SendRawEmail'],
        resources: ['*'], // SES resources are region-specific
      }),
    ...
    adminProducts.addMethod(
      'GET',
      new apigateway.LambdaIntegration(adminListProductsFunction),
    )
    adminProducts.addMethod(
      'POST',
    ...
    adminProductIndexes.addMethod(
      'PATCH',
      new apigateway.LambdaIntegration(adminUpdateProductIndexesFunction),
    )

    const adminProduct = adminProducts.addResource('{id}')
    adminProduct.addMethod(
      'GET',
    ...
    adminCategories.addMethod(
      'GET',
      new apigateway.LambdaIntegration(listCategoriesFunction),
    )
    adminCategories.addMethod(
      'POST',
      new apigateway.LambdaIntegration(createCategoryFunction),
    )

    const adminCategoryIndexes = adminCategories.addResource('indexes')
    adminCategoryIndexes.addMethod(
      'PATCH',
      new apigateway.LambdaIntegration(adminUpdateCategoryIndexesFunction),
    )

    const adminCategory = adminCategories.addResource('{id}')
    adminCategory.addMethod(
      'GET',
      new apigateway.LambdaIntegration(getCategoryFunction),
    )
    adminCategory.addMethod(
      'PUT',
      new apigateway.LambdaIntegration(updateCategoryFunction),
    )
    adminCategory.addMethod(
      'DELETE',
      new apigateway.LambdaIntegration(deleteCategoryFunction),
    )

    const adminOrders = admin.addResource('orders')
    adminOrders.addMethod(
      'GET',
      new apigateway.LambdaIntegration(listOrdersFunction),
    )

    const adminOrder = adminOrders.addResource('{id}')
    adminOrder.addMethod(
      'GET',
      new apigateway.LambdaIntegration(getOrderFunction),
    )

    const adminOrderStatus = adminOrder.addResource('status')
    adminOrderStatus.addMethod(
      'PUT',
      new apigateway.LambdaIntegration(updateOrderStatusFunction),
    )
    ...
    pingError.addMethod(
      'GET',
      new apigateway.LambdaIntegration(pingErrorFunction),
    )
...
function handler(event) {
  ...
  return request;
    ...
    new s3deploy.BucketDeployment(this, 'DeployDocs', {
      sources: [
        s3deploy.Source.asset('infrastructure', {
