...
const mockGetAllCategories = mock<(status?: 'active' | 'inactive') => Promise<any[]>>(() => Promise.resolve([]));
...
describe('Admin List Categories Handler (unit)', () => {
  ...
  test('applies status filter, search, sorting and pagination', async () => {
    ...
    const event = {
      queryStringParameters: { status: 'active', q: 'a', sort: '-title', limit: '2', offset: '0' },
      requestContext: { domainName: 'example.com', path: '/admin/categories' }
    } as unknown as APIGatewayProxyEvent;
  ...
  test('paginates results and provides next/prev links', async () => {
    const categories = [];
    for (let i = 0; i < 10; i++) categories.push({ id: `${i}`, title: `Item ${i}`, index: i, createdAt: i });
  ...
  test('matches snapshot for happy path', async () => {
    const categories = [
      { id: '1', title: 'Snapshot Cat 1', index: 1, createdAt: 1234567890, status: 'active' },
      { id: '2', title: 'Snapshot Cat 2', index: 2, createdAt: 1234567890, status: 'inactive' },
    ];
