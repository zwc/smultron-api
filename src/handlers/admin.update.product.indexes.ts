...
import { updateProductIndex } from '../services/product';
  ...
  try {
    ...
    const results = await Promise.allSettled(
      validatedBody.updates.map(update => 
        updateProductIndex(update.id, update.index)
      )
    );
    ...
    if (failures.length > 0) {
      ...
      console.error('Failed to update some product indexes:', errorDetails);
      return errorResponse(`Failed to update ${failures.length} product index(es)`, 400);
    }

    // Return success with count of updated products
    return successResponse({
      updated: validatedBody.updates.length,
      message: `Successfully updated ${validatedBody.updates.length} product index(es)`
    });
